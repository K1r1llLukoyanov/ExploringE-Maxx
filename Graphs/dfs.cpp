...
{                // v - текущая вершина, comp - компонента связности
...
int find_components_and_topology()
{ // поиск компонент связности ( то есть все вершины, такие что из одной любой можно добраться в любую другую)
    for (int i = 0; i < n; i++)
